"""B站平台解析器"""

from typing import Dict, Any
from playwright.async_api import Page
from pathlib import Path
from datetime import datetime

from .base import BaseParser
from workflows.services import get_logger

logger = get_logger()


class BilibiliParser(BaseParser):
    """B站平台解析器"""

    def __init__(self):
        super().__init__("B站")

    async def parse(self, page: Page, url: str) -> Dict[str, Any]:
        """解析B站页面"""
        await self.wait_for_content(page)

        # 标题
        title = await self._safe_text(page, "h1")
        if not title:
            title_text = await self._safe_text(page, "title")
            if title_text:
                title = title_text.replace("_哔哩哔哩_bilibili", "").strip()

        # 发布日期
        publish_date = await self._extract_date(page)

        # 文章类型：B站默认视频
        article_type = "视频"

        # 截图
        screenshot_path = await self._take_screenshot(page, url)

        return {
            "title": title or "未能提取标题",
            "publish_date": publish_date,
            "article_type": article_type,
            "screenshot_path": screenshot_path
        }

    async def _safe_text(self, page: Page, selector: str) -> str:
        """安全地提取文本"""
        try:
            text = await page.text_content(selector, timeout=3000)
            return text.strip() if text else ""
        except Exception:
            return ""

    async def _extract_date(self, page: Page) -> str:
        """提取发布日期"""
        try:
            # 方法1: .pubdate-ip-text class
            date_elem = await page.query_selector(".pubdate-ip-text")
            if date_elem:
                text = await date_elem.inner_text()
                # 格式: "2026-01-22 12:31:51"
                if text and len(text) >= 10:
                    return text[:10]  # 只取日期部分
        except Exception as e:
            logger.warning("bilibili_date_extract_failed", error=str(e))

        return datetime.now().strftime("%Y-%m-%d")

    async def _take_screenshot(self, page: Page, url: str) -> str:
        """截图"""
        import hashlib
        url_hash = hashlib.md5(url.encode()).hexdigest()[:8]
        filename = f"bilibili_{url_hash}.png"
        filepath = Path("screenshots") / filename
        filepath.parent.mkdir(exist_ok=True)
        
        await page.screenshot(path=str(filepath), full_page=False)
        return str(filepath)
