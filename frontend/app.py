...
if page == "首页概览":
    ...
    c1.metric("任务状态", "已完成", "本次文件 1 / 1")
    c2.metric("媒体数量", "128", "已完成匹配")
    c3.metric("约稿数量", "256", "图文 + 视频")
    c4.metric("费用总额", "¥231,600", "1 项待确认", delta_color="inverse")
    st.subheader("业务处理流程")
    ...
    if st.session_state.processing_stage == "running" and st.session_state.backend_task_id:
        ...
        if st.button("刷新处理状态", width="content"):
            try:
    ...
    level = f1.selectbox("媒体等级", ["全部"] + sorted(DETAILS["媒体等级"].unique().tolist()))
    platform = f2.selectbox("发布平台", ["全部"] + sorted(DETAILS["发布平台"].unique().tolist()))
    state = f3.selectbox("状态", ["全部"] + sorted(DETAILS["状态"].unique().tolist()))
    ...
    st.caption(f"当前显示 {len(filtered)} 条演示记录；完整任务共 256 条约稿数据。")
    ...
    c1.metric("本次总费用", "¥231,600")
    c2.metric("本次图文费用", "¥153,600", "66.3%")
    c3.metric("本次视频费用", "¥78,000", "33.7%")
    st.subheader("当月 TOP 媒体费用")
    st.caption("汇总系统当月保存的约稿记录，并按媒体费用从高到低展示。")
    top = DETAILS.nlargest(5, "金额")[["媒体名称", "金额"]]
    cost_bar_chart(top, "媒体名称")

    st.divider()
    st.subheader("当月费用汇总")
    st.caption("系统保存每次上传的约稿处理结果，并按自然月自动累计统计。当前展示为模拟历史批次数据。")
    ...
    st.dataframe(
        monthly_table[["上传日期", "处理批次", "约稿数量", "图文费用", "视频费用", "费用总额"]],
        width="stretch",
        hide_index=True,
        column_config={
